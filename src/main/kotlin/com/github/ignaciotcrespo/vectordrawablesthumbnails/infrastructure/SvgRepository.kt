...
/**
 * Repository implementation for SVG files.
 * Follows the Single Responsibility Principle by handling only SVG format.
...
class SvgRepository(
    ...
    override fun loadVectors(project: Project): Observable<VectorItem> {
        return fileSearcher.searchVectorFiles(project, setOf(FileType.SVG))
            .subscribeOn(Schedulers.io())
            .observeOn(Schedulers.newThread())
            .flatMap { validFile -> parser.parseVectorFile(validFile) }
    }

    override fun getFileType(): String = "SVG"
}
